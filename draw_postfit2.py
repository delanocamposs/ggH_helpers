...
def run(mass, ctau, year):
    ...
    ggHfitter.fitSIGBKG(f"sig_bkg_summary_histos_m{mass}_ct{ctau}_year{year}.root", "signal_hist", "data_hist", f"SB_fit_result_m{mass}_ct{ctau}_year{year}.root", order=4)
    SB_file=ROOT.TFile.Open(f"SB_fit_result_m{mass}_ct{ctau}_year{year}.root")
    print("got here")
    w = SB_file.Get("w")
    x= w.var("mass")
    pdfsig = wsig.pdf("model")
    pdfbkg = wbkg.pdf("model")
    ...
    can.cd()
    can.Update()
    can.SaveAs(f"summary_plot_{cat}_m{mass}_ct{lifetime}_{year}.png")
    can.SaveAs(f"summary_plot_{cat}_m{mass}_ct{lifetime}_{year}.pdf")
